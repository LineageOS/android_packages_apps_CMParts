    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mCustomBackground){
            if (mCustomBackground.findIndexOfValue(val) == 0){
                ColorPickerDialog cp = new ColorPickerDialog(this,mPackageColorListener,Settings.System.getInt(getContentResolver(), 
                ...
                intent.putExtra("outputX", getWindowManager().getDefaultDisplay().getWidth()/2);
                intent.putExtra("outputY", getWindowManager().getDefaultDisplay().getHeight()/2);
                File lockWall = new File(getApplicationContext().getFilesDir()+"/lockwallpaper");
                if (!lockWall.exists()){
                    try {
