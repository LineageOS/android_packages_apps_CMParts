...
public class GestureCreateActivity extends Activity {
    ...
    private class GesturesProcessor implements GestureOverlayView.OnGestureListener {
        ...
        public void onGestureEnded(GestureOverlayView overlay, MotionEvent event) {
            ...
            if (mGesture.getStrokesCount() == 1) { // check vertical state - can't be less than 45 deg from vertical
                RectF box = mGesture.getBoundingBox(); // I'd like to fix this but for now... keep people from making
                if (Math.abs(box.width() / box.height()) < 1.0) { // gestures they can't reproduce on the lockscreen
                    overlay.clear(false);
                }
            }
            mDoneButton.setEnabled(true);
