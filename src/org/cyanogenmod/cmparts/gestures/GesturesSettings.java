...
public class GesturesSettings extends SettingsPreferenceFragment {
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        mDefaultActions = getContext().getResources()
        ...
        if (mDefaultActions.length < mGestures.length) {
            mDefaultActions = new int[mGestures.length];
        }
    ...
    private void initTouchscreenGesturesCategory(final PreferenceCategory category) {
        ...
        for (final TouchscreenGesture gesture : mGestures) {
            category.addPreference(new TouchscreenGesturePreference(
                    getContext(), gesture, mDefaultActions[gesture.id]));
        }
    }
    ...
    public static void restoreTouchscreenGestureStates(final Context context) {
        ...
        int[] defaultActions = context.getResources()
                .getIntArray(R.array.config_defaultTouchscreenGestureActions);
        // Do not accept the config if there aren't enough default actions
        if (defaultActions.length < mGestures.length) {
            defaultActions = new int[mGestures.length];
        }
