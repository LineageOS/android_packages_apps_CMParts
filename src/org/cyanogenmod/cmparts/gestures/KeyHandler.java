...
import org.cyanogenmod.cmparts.gestures.MovesSettings;
...
public class KeyHandler implements DeviceKeyHandler {
    ...
    private static final int ACTION_FLASHLIGHT = 2;
    ...
    private static final int ACTION_PLAY_PAUSE_MUSIC = 7;
    private static final int ACTION_PREVIOUS_TRACK = 8;
    private static final int ACTION_NEXT_TRACK = 9;
    ...
    public boolean handleKeyEvent(final KeyEvent event) {
        ...
        final boolean supported = MovesSettings.sGestureActionMap.containsKey(code);
        if (!supported) {
            return false;
        }

        if (!mEventHandler.hasMessages(GESTURE_REQUEST)) {
            final Message msg = getMessageForKeyEvent(code);
    ...
    private void processEvent(final int code) {
        ...
        mSensorManager.registerListener(new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent event) {
                ...
                if (event.values[0] == mProximitySensor.getMaximumRange()) {
                    Message msg = getMessageForKeyEvent(code);
                    mEventHandler.sendMessage(msg);
                }
            }
    ...
    private Message getMessageForKeyEvent(final int code) {
    ...
    private class EventHandler extends Handler {
        ...
        public void handleMessage(final Message msg) {
            final int action = MovesSettings.sGestureActionMap.get(msg.arg1);
            switch (action) {
                case ACTION_CAMERA:
                    launchCamera();
                    break;
