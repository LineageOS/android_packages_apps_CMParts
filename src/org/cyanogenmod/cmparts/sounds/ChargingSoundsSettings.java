...
public class ChargingSoundsSettings extends SettingsPreferenceFragment implements
	...
	final String toneName;
        ...
        if (toneUriString != null && !toneUriString.equals(RINGTONE_SILENT_URI_STRING)) {
            final Ringtone ringtone = RingtoneManager.getRingtone(getActivity(), Uri.parse(toneUriString));
            if (ringtone != null) {
                toneName = ringtone.getTitle(getActivity());
            } else {
                // Unlikely to ever happen
                // (but is possible if the ringtone previously
                //  chosen is removed during upgrade)
                toneName = "";
                toneUriString = System.DEFAULT_NOTIFICATION_URI.toString();
                persist = true;
            }
        } else {
            // Silent
    ...
    private void launchNotificationSoundPicker(int code, String toneUriString) {
        ...
        if (toneUriString != null &&
                !toneUriString.equals(RINGTONE_SILENT_URI_STRING)) {
